...
export class UserRepository extends PgRepository implements LoadUsers, ShowUser, UpdateUser, DeleteUser, InsertUser, ShowUserByNickName {
  async get (input: GetInput): Promise<GetOutput> {
    return await this.getRepository(User).find(input !== undefined ? { order: { name: 'ASC' }, select: ['id', 'name', 'nickname'], relations: ['questions', 'questions.status', 'notifications', 'games', 'games.question', 'games.award', 'games.help', 'games.finished', 'games.option'], where: { id: Not(input.id) } } : { order: { name: 'ASC' }, select: ['id', 'name', 'nickname'], relations: ['questions', 'questions.status', 'notifications', 'games', 'games.question', 'games.award', 'games.help', 'games.finished', 'games.option'] })
  }

  async show ({ id }: ShowInput): Promise<ShowOutput> {
    return await this.getRepository(User).findOne(id, { select: ['id', 'name', 'nickname'], relations: ['questions', 'notifications', 'games'] })
  }
